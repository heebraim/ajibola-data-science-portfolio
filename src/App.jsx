...
const App = () => {
  ...
  // Helper to close nav on mobile link click
  const handleNavClick = () => setNavOpen(false);

  // Mobile nav link component for reusability
  const MobileNavLink = ({ to, children }) => (
    <ScrollLink to={to} smooth={true} className="block py-2" onClick={handleNavClick}>
      {children}
    </ScrollLink>
  );
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark' : ''}`}>
      <div
        className={`${darkMode ? colors.dark.background : colors.light.background} ${
          darkMode ? colors.dark.text : colors.light.text
        }`}
      >
        {/* Navigation */}
            ...
            {projects.map((project, index) => (
              ...
              >
                ...
                <ul className="mb-4 space-y-2">
                  {project.tech.map((tech, i) => (
                    <li key={i} className="flex">
                      <span className="flex-shrink-0 w-2 h-2 mt-2 mr-2 bg-blue-500 rounded-full"></span>
                ...
                <div className="flex space-x-4">
                  <a href={project.links.preview} className="text-blue-500">View Project</a>
